...
public class BlocksGenerator extends AbstractMojo {
    ...
    private void doGenerate() throws IOException, FormatterException {
        ...
        classOutput.append("package com.github.steveice10.mc.protocol.data.game.world.block;").append(System.lineSeparator());
        classOutput.append("import com.github.steveice10.mc.protocol.data.game.world.block.property.*;").append(System.lineSeparator());
        classOutput.append("import static com.github.steveice10.mc.protocol.data.game.world.block.BlockDef.*;").append(System.lineSeparator());

        classOutput.append("/**").append(System.lineSeparator()).append(" * Automatically created from mojangs reports").append(System.lineSeparator()).append("*/");
        classOutput.append("public class Blocks {");

        methodOutput.append("package com.github.steveice10.mc.protocol.data.game.world.block;").append(System.lineSeparator());
        methodOutput.append("import com.github.steveice10.mc.protocol.data.game.world.block.property.*;").append(System.lineSeparator());
        methodOutput.append("/**").append(System.lineSeparator()).append(" * Automatically created from mojangs reports").append(System.lineSeparator()).append("*/");
                ...
                for (Map.Entry<Set<String>, String> x : new HashMap<>(map).entrySet()) {
                    if (names.containsKey(x.getValue())) {
                        ...
                        if (x.getKey().containsAll(names.get(x.getValue()))) {
                            ...
                            System.out.println("Fair warning: Expanding " + Util.reverse(x.getValue()) + " with values: " + names.get(x.getValue()) + " to " + x.getKey());
                            ...
                            // x.getKey() is the upper one?
                            System.out.println("Fair warning: Expanding " + Util.reverse(x.getValue()) + " with values: " + x.getKey() + " to " + names.get(x.getValue()));
//                            x.getKey().addAll(names.get(x.getValue()));
                        ...
                        throw new IllegalStateException("Duplicate enum name: " + Util.reverse(x.getValue()) + " with values: " + x.getKey() + "/" + names.get(x.getValue()));
                    }
            ...
            for (Map.Entry<String, JsonElement> blockE : blocksReport.entrySet()) {
                ...
                if (states.size() > 20) {
                    ...
                    methodOutput
                            ...
                            .append(Util.toCamel(shortName))
                            .append("(){").append(System.lineSeparator())
                            .append(" return com.github.steveice10.mc.protocol.data.game.world.block.")
...
//                }
                ...
                int total_n = -1;
                final int max_per_fnc = 90;
                int brackets_to_close = 0;
                for (int n = 0, i = states.size(); i > max_per_fnc; i -= max_per_fnc) {
...
//                }
                ...
                for (JsonElement stateE : states) {
                    ...
                    if (state.has("properties")) {
                        ...
                        for (Map.Entry<String, JsonElement> stateProperty : properties.entrySet()) {
                            stateMethodOutput.append(".with( Blocks.").append(shortName).append("_").append(stateProperty.getKey().toUpperCase())
                ...
                if (sepeareClassName == null) {
                    ...
                    sb.append("package com.github.steveice10.mc.protocol.data.game.world.block;").append(System.lineSeparator());
                    sb.append("import com.github.steveice10.mc.protocol.data.game.world.block.property.*;").append(System.lineSeparator());

                    sb.append("/**").append(System.lineSeparator()).append(" * Automatically created from mojangs reports").append(System.lineSeparator()).append("*/");
                    ...
                    Util.writeFile(new File(sourceDirectory, "com/github/steveice10/mc/protocol/data/game/world/block/" + sepeareClassName + ".java"),
                            sb.toString());
        ...
        Util.writeFile(new File(sourceDirectory, "com/github/steveice10/mc/protocol/data/game/world/block/Blocks.java"),
        ...
        Util.writeFile(new File(sourceDirectory, "com/github/steveice10/mc/protocol/data/game/world/block/BlockDef.java"),
                methodOutput.toString());

        // Write the formated again
        Util.writeFile(new File(sourceDirectory, "com/github/steveice10/mc/protocol/data/game/world/block/BlockDef.java"),
                formatter.formatSource(methodOutput.toString()));
        System.out.println("Generated all Blocks and BlockStates with the heighest id being " + maxID + " (don't forget to +1)");
    }
    ...
    private void prepareBlockProperty(String blockName, String name, JsonArray jsonValues) {
        ...
        if (!(values.size() == 2 && values.contains("true") && values.contains("false"))
                ...
                && !(Util.isListOf(values, "down", "up", "north", "south", "west", "east", "special") && !values.contains("side"))) {
            ...
            if (other != null) {
                ...
                if (!pref.isEmpty()) {
                    ...
                    throw new IllegalStateException("Can't handle custom enum creation for " + values.toString() + " and property name " + name + "//"
    ...
    private void generateProperty(StringBuilder out, String blockName, String name, JsonArray jsonValues) throws FormatterException, IOException {
        ...
        if (values.size() == 2 && values.contains("true") && values.contains("false")) {
            ...
            String varName = blockName + "_" + name.toUpperCase();
            writeEnumProperty(out, varName, name, "com.github.steveice10.mc.protocol.data.game.world.Axis", values);
        } else if (Util.isListOf(values, "down", "up", "north", "south", "west", "east", "special") && !values.contains("side")) {
            // Generate BlockFace enum property
            String varName = blockName + "_" + name.toUpperCase();
            writeEnumProperty(out, varName, name, "com.github.steveice10.mc.protocol.data.game.world.block.BlockFace", values);
        } else {
            ...
            String fullClazz = "com.github.steveice10.mc.protocol.data.game.world.block.property.enums." + clazzName;
            String varName = blockName + "_" + name.toUpperCase();
            ...
            Util.writeFile(new File(sourceDirectory, Util.packageToPath(fullClazz) + ".java"), writeEnumFile(clazzName, values));
        }
    }

    private void writeEnumProperty(StringBuilder out, String variableName, String propertyName, String enumClass, Collection<String> values) {
        out.append("public final static BlockPropertyEnum<").append(enumClass).append("> ")
                .append(variableName)
                .append(" = BlockPropertyEnum.create(\"").append(propertyName).append("\", ").append(enumClass).append(".class");
        for (String x : values) {
    ...
    private String writeEnumFile(String clazzName, Collection<String> values) throws FormatterException {
        ...
        ret.append("package com.github.steveice10.mc.protocol.data.game.world.block.property.enums;").append(System.lineSeparator());
        ret.append("public enum ").append(clazzName).append(" implements com.github.steveice10.mc.protocol.data.game.world.block.property.IBlockPropertyEnum {")
                .append(System.lineSeparator());
